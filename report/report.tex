...
Para aplicar aprendizaje por refuerzo (RL), modelamos la construcción de la ruta del comerciante como un problema de toma de decisiones secuencial.
...
\begin{itemize}
    ...
    \item Procesamiento: Un Perceptrón Multicapa (MLP) con funciones de activación ReLU.
...
\begin{en}
1.  Se calcula una máscara de validez $M_t \in \{0, 1\}^N$.
2.  Los logits correspondientes a puertos inválidos se establecen en $-\infty$.
3.  Se aplica una función \textbf{Softmax} para obtener una distribución de probabilidad válida:
    \[ \pi_\theta(v_{t+1} | v_t, \mathcal{I}) = \text{Softmax}(\text{Logits}) \]

\subsubsection{Especificaciones Técnicas (V1)}

La implementación inicial en PyTorch se estructura con:
\begin{itemize}
    \item \textbf{Hiperparámetros Base}: Embedding Dim: 128, Hidden Dim: 1024, Optimizador Adam ($10^{-4}$).
    \item \textbf{Encoder}: Linear $\to$ ReLU $\to$ Linear.
    \item \textbf{Decoder}: Linear $\to$ ReLU $\to$ Linear.
\end{itemize}

\subsection{Algoritmo de Entrenamiento Base: REINFORCE}

El entrenamiento base utiliza el algoritmo de gradiente de política \textbf{REINFORCE}, optimizando la esperanza del beneficio $J(\theta) = \mathbb{E}_{\tau \sim \pi_\theta} [R(\tau)]$.
...
Implementamos una estrategia de \textbf{Temperature Scaling} en la función Softmax:
...
2. Realiza múltiples iteraciones sobre la \textbf{misma instancia}.
3. Mantiene un registro de la mejor solución encontrada.
...
\begin{itemize}
    \item \textbf{Dimensiones Aumentadas}: Embedding: $128 \to 512$, Hidden: $1024 \to 2048$.
    \item \textbf{Profundidad Adicional}: Capas adicionales en el codificador de contexto y decodificador para permitir un razonamiento más abstracto.
\end{itemize}

\subsubsection{Memory Replay: Refuerzo de Soluciones Élite}
Se incorpora un \textbf{buffer de memoria} que almacena las $K=4$ mejores rutas encontradas.
