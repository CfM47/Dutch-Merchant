...
Para abordar la complejidad combinatoria de la selección de rutas en instancias de gran escala, se propone un modelo de aprendizaje por refuerzo basado en gradiente de política (Policy Gradient), específicamente utilizando el algoritmo REINFORCE \cite{russell2020}. Este enfoque permite que un agente aprenda a construir secuencias de puertos que maximicen el potencial de ganancia, adaptándose a las restricciones temporales y operativas de forma dinámica.
...
Para verificar si la diferencia de rendimiento entre los métodos es estadísticamente significativa, se aplicó una prueba t-student para muestras relacionadas (paired t-test) \cite{ross2014}. El diseño del test se fundamenta en las siguientes premisas: 
...
En todos los experimentos realizados (Tabla \ref{tab:inferencial}), el p-valor resultante fue consistentemente superior a $0.05$. Bajo este criterio estadístico, no se dispone de evidencia suficiente para rechazar la hipótesis nula \cite{ross2014}. Este resultado es de gran relevancia práctica, ya que permite afirmar con rigor estadístico que la aproximación mediante Reinforcement Learning no presenta una degradación significativa de la calidad de la solución frente al óptimo global en las instancias evaluadas, ofreciendo además una escalabilidad superior para problemas de mayor dimensión.
