...
pub fn calculate_best_profit(instance: &Instance, nodes: &[PortId]) -> f64 {
    if nodes.is_empty() {
        return instance.initial_capital;
    }

    let r = nodes.len(); // number of ports in route
    let m = instance.n_goods; // number of goods
    ...
    match problem.solve() {
        ...
        Err(_) => instance.initial_capital, // If infeasible, return initial capital
    }
}
