...
class Instance(BaseModel):
    ...
    travel_cost: float = Field(..., description="Travel cost per unit of time")

    # w(m): weight of good m
    # Shape: [n_goods]
    weight: List[float] = Field(..., description="Weight of each good type")
    ...
    # Shape: [n_ports]
    visit_cost: List[float] = Field(..., description="Cost S(v) of visiting port v")

    # v_0: Initial port
    start_port: PortId = Field(..., description="Initial port ID (v_0)")

    # B: Boat capacity
    capacity: float = Field(..., description="Boat capacity (B)")

    # T: Time limit
    time_limit: float = Field(..., description="Time limit (T)")

    # f_0: Initial capital
    initial_capital: float = Field(..., description="Initial capital (f_0)")
